...
namespace eShop.Data.Static
{
    public class ProductRepository : IProductRepository
    {
        ...
        {
            return Task.FromResult(_products.SingleOrDefault(product => product.Id == productId));
        }

        public Task<IEnumerable<Product>> GetProductsAsync(string filter = null)
        {
            return string.IsNullOrWhiteSpace(filter)
                ? Task.FromResult<IEnumerable<Product>>(_products)
                : Task.FromResult(
                    _products.Where(product => product.Name.IndexOf(filter, StringComparison.OrdinalIgnoreCase) >= 0));
        }
    }
}